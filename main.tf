...
# Create Instuctor Chef Server

resource "aws_instance" "instructor_chef_server" {
  connection {
    type        = "ssh"
    user        = "${var.aws_ami_user}"
    private_key = "${file("${var.aws_key_path}")}"
  ...
  ami                         = "${var.aws_ami}"
  count                       = 1
  instance_type               = "${var.instructor_chef_server_instance_type}"
  key_name                    = "${var.aws_key_name}"
  subnet_id                   = "${aws_subnet.auar_public_subnet.id}"
  vpc_security_group_ids      = ["${aws_security_group.auar_allow_all.id}"]
  ...
  provisioner "remote-exec" {
    inline    = [
      "sudo hostnamectl set-hostname instructor.chefserver.success.chef.co",
      "sudo /usr/bin/yum -y install wget",
      "sudo /bin/wget https://packages.chef.io/files/stable/chefdk/1.3.43/el/7/chefdk-1.3.43-1.el7.x86_64.rpm -O /tmp/chefdk-1.3.43-1.el7.x86_64.rpm",
      "sudo /bin/rpm -Uv /tmp/chefdk-1.3.43-1.el7.x86_64.rpm",
      "sudo /bin/chef-solo -c /tmp/cookbooks/solo.rb -o recipe[automate_lab],recipe[automate_lab::instructor_chef_server]"
    ]
  }
  ...
  tags {
  Name      = "automate-up-and-running-training-instructor-chef-server"
  X-Dept    = "Success"
  X-Contact = "jobrien"
  }
}

# Create Instuctor Automate Servers

resource "aws_instance" "instructor_automate_server" {
  connection {
    type        = "ssh"
    user        = "${var.aws_ami_user}"
    private_key = "${file("${var.aws_key_path}")}"
  ...
  ami                         = "${var.aws_ami}"
  count                       = 1
  instance_type               = "${var.instructor_automate_instance_type}"
  key_name                    = "${var.aws_key_name}"
  subnet_id                   = "${aws_subnet.auar_public_subnet.id}"
  vpc_security_group_ids      = ["${aws_security_group.auar_allow_all.id}"]
  ...
  provisioner "remote-exec" {
    inline    = [
      "sudo hostnamectl set-hostname instructor.automate.success.chef.co",
      "sudo /usr/bin/yum -y install wget",
      "sudo /bin/wget https://packages.chef.io/files/stable/chefdk/1.3.43/el/7/chefdk-1.3.43-1.el7.x86_64.rpm -O /tmp/chefdk-1.3.43-1.el7.x86_64.rpm",
      "sudo /bin/rpm -Uv /tmp/chefdk-1.3.43-1.el7.x86_64.rpm",
      "sudo /bin/chef-solo -c /tmp/cookbooks/solo.rb -o recipe[automate_lab],recipe[automate_lab::instructor_automate]"
    ]
  }
  ...
  tags {
  Name      = "automate-up-and-running-training-instructor-automate-server"
  X-Dept    = "Success"
  X-Contact = "jobrien"
  }
}

# Create Instuctor Chef Runners

resource "aws_instance" "instructor_runner" {
  connection {
    type        = "ssh"
    user        = "${var.aws_ami_user}"
    private_key = "${file("${var.aws_key_path}")}"
  ...
  ami                         = "${var.aws_ami}"
  count                       = 1
  instance_type               = "${var.instructor_runner_instance_type}"
  key_name                    = "${var.aws_key_name}"
  subnet_id                   = "${aws_subnet.auar_public_subnet.id}"
  vpc_security_group_ids      = ["${aws_security_group.auar_allow_all.id}"]
  ...
  provisioner "remote-exec" {
    inline    = [
      "sudo hostnamectl set-hostname instructor.runner.success.chef.co",
      "sudo /usr/bin/yum -y install wget",
      "sudo /bin/wget https://packages.chef.io/files/stable/chefdk/1.3.43/el/7/chefdk-1.3.43-1.el7.x86_64.rpm -O /tmp/chefdk-1.3.43-1.el7.x86_64.rpm",
      "sudo /bin/rpm -Uv /tmp/chefdk-1.3.43-1.el7.x86_64.rpm",
      "sudo /bin/chef-solo -c /tmp/cookbooks/solo.rb -o recipe[automate_lab]"
  ...
  tags {
  Name      = "automate-up-and-running-training-instructor-runner"
  X-Dept    = "Success"
  X-Contact = "jobrien"
  }
}
